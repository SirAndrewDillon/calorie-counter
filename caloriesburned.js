...
/* Created by: https://github.com/SirAndrewDillon
  ...
  if (event.keyCode === 13) {
    if (event.target.value < 10) {
      document.getElementById(
        'alert-value'
      ).innerHTML = `The weight you entered must be greater than 10 ${weightMetric}!`;
    ...
    document
      .getElementById('weight-input-edit')
      .addEventListener('keyup', event => {
        if (event.keyCode === 13) {
          ...
          activityChange.forEach(x => {
            updateValues(x, activityMetric);
          });
        }
      });
  }
...
const facts = [
  {
    ...
    comment: 'Is this a fun fact or is it useless?'
  },
  {
    fact: 'Singing in the shower can burn an extra 10-20 Calories per song',
    comment: "So sing like nobody is watching because they arn't"
  },
  {
    fact:
      'Laughing for 10 minutes can make you burn between 20 and 40 Calories',
    comment: 'Heard any good jokes lately?'
  },
  {
  ...
  {
    fact:
      'Calories keep you alive, help you build new tissue, and give you energy to move around.',
  ...
  {
    fact:
      'You burn dozens of calories per hour while sleeping, and even more during REM sleep',
    comment: 'Who knew taking a nap could be such a great workout?'
  },
  {
    fact:
      '10 lbs of muscle burns 50 calories a day at rest, while 10 lbs of fat only burns 20',
...
const changeInnerHtml = (id, change) => {
  document.getElementById(id).innerHTML = ` ${change}`;
};
const calculateBurn = ({ cal, amount }, { weight }) =>
  parseInt(cal * weight * amount, 10);
...
const optionsToggle = () => {
  ...
  document.addEventListener('click', e => {
    // Clicking outside
    if (
      e.target.id !== 'speedOption' &&
      options.classList.contains('speed-options-open')
    ) {
...
const updateValues = (a, m) => {
  ...
  document.getElementById(
    'speedValue'
  ).innerHTML = `${currentSpeed} ${speedPerHour}<div class='arrow metric-change mph-drop' id="metric-dropdown"></div>`;
  let value;
...
const addEventForCals = activityData => {
  ...
  document.getElementById(`${id}-input`).addEventListener('keyup', event => {
    ...
    const distanceValue =
      activityMetric === 'mi'
        ? event.target.value
        : convertKmToMiles(event.target.value);
...
['walking0', 'running1', 'jumpingjacks2'].forEach(x => {
  addEventForCals(x);
});
const changeMetric = () => {
...
dropdown.addEventListener('click', () => {
  weightMetric = dropdown.innerHTML;
  changeMetric();
  closeMetricDropdown();
});
...
const updateInputWeight = nw => {
  document.getElementById('weight-input-edit').value = `${removeChar(
    nw
  )}${weightMetric}`;
};
document.getElementById('weightInputMetric').addEventListener('click', () => {
  weightMetricDropdownToggle();
});
const editWeight = () => {
  const editweightInput = document.getElementById('weight-input-edit');
  document.getElementById(
    'buttonsContainer'
  ).innerHTML = `<div class="edit-button cancel" onclick="cancelEdit()">Cancel Edit</div><div class="edit-button save" onclick="saveEdit()">Save</div>`;
  console.log('editweight running');
  prevWeight =
    editweightInput.value !== '' ? editweightInput.value : weightInput.value;
  editweightInput.readOnly = false;
  editweightInput.focus();
  editweightInput.value = '';
  const pounds = parseInt(event.target.value, 10); // Getting the weight the user inputed
  document.getElementById('weightInputMetric').innerHTML =
    '<p class="cancel" id="cancelButton">cancel</p>';
  document
    .getElementById('cancelButton')
    .addEventListener('click', () => cancelEdit()); // Add Event For Edit Button
};
const cancelEdit = () => {
  const editweightInput = document.getElementById('weight-input-edit');
  changeInnerHtml(
    'buttonsContainer',
    '<p class="edit-button" onclick="editWeight()" id="editButton">Change Weight</p>'
  );
  ...
  document
    ...
    .addEventListener('click', () => editWeight()); // Add Event For Edit Button
};
const saveEdit = () => {
  const editweightInput = document.getElementById('weight-input-edit');
...
const updateRunBurn = () => {
  ...
  running.cal *= speed;
  document.getElementById('calorierunning').innerHTML = ` ${calculateBurn(
    running,
    user
  )}`;
...
let activityhigh = 'walking';
let activitymed = 'running';
let activitylow = 'jumping jacks';
document.getElementById('generateRoutine').addEventListener('click', () => {
  executeRoutine();
});

document
  .getElementById('splitPriority')
  .addEventListener('click', () => splitPriority());
document.getElementById(
  `${removeSpace(activityhigh)}_high`
).style.backgroundColor = priorityColor;
document.getElementById(
  `${removeSpace(activitymed)}_med`
).style.backgroundColor = priorityColor;
document.getElementById(
  `${removeSpace(activitylow)}_low`
).style.backgroundColor = priorityColor;
activities.forEach(({ activity }) => {
  prioritiesArr.forEach(priority => {
    const button = document.getElementById(
      `${removeSpace(activity)}_${priority}`
    );
...
const splitPriority = () => {
  activities.forEach(x => {
    const y = x;
    y.percentage = 100 / activities.length;
  });
};
splitPriority();
...
const changingPriority = (activity, priority) => {
  ...
  ['high', 'med', 'low'].forEach(x => {
    if (
      document.getElementById(`${removeSpace(activity)}_${x}`).style
...
const createRoutine = ({ activity, metric, cal, percentage }) => {
  ...
  return `<div>${Math.round(amountNeeded * 10) /
    10} ${metric}s of ${activity}: ${Math.round(calsBurned * 10) / 10} <div>`;
};
const executeRoutine = () => {
  document.getElementById('showResults').innerHTML = activities
    .flatMap(createRoutine)
    .join('');
};
const targetCal = () =>
  document.getElementById('target-cal').addEventListener('keyup', event => {
    if (event.keyCode === 13) {
